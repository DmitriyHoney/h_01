...
const isPayloadValid = (payload, schema) => {
    ...
    const checkSimpleType = (key, value) => {
        ...
        if (isHardType)
            return;
        // @ts-ignore
        if (schema[key].hasOwnProperty('default') && value === schema[key].default)
            return;
    ...
    const collectAndInitDefault = (payload) => {
        const defaultFields = Object.assign({}, payload);
        const setDateByPattern = (defaultPattern, depField = '') => {
            if (defaultPattern === '_:_:_T_:_:_')
        ...
        Object.entries(schema).forEach(([key, value]) => {
            ...
            if (!isDefaultAndNotExistInPayload)
                return;
            value.type === 'date'
                // @ts-ignore
